...
const Home: NextPage = () => {
  ...
  const [contractST, setContractST] = React.useState<any>(null);
  const [contractSTS, setContractSTS] = React.useState<any>(null);
  ...
  const [STtoETH, setSTtoETH] = React.useState<any>(null);
  const [myST, setMyST] = React.useState<number>(0);
  ...
  const handleUseEffect = async () => {
    ...
    load().then((e) => {
      ...
      setContractST(e.contractST);
      setContractSTS(e.contractSTS);
      setTokensSold(e.tokensSold);
      setTransactionCount(e.transactionCount);
      setEthFunds(e.ethFunds);
      const stETH = (50 / e.ethPriceN).toFixed(2);
      setSTtoETH(stETH);
  ...
  // Functions
  const isLoading = () => account == null || contractST == null || contractSTS == null || tokensSold == null || transactionCount == null || ethFunds == null || STtoETH == null;
  const SliderValue = () => {
    const bigInt = 750000 * 10**18;
    const r = ((tokensSold * 100) / bigInt);
    return r;
  };
  const buyTokens = async () => {
    const big = BigInt(inputValue * 10**18);
    await contractSTS.buyToken(big, {
      from: account,
      value: inputValue * STtoETH * 10**18,
  ...
  return (
    ...
    <VStack minH='800px' w='full' bg={bg}>
      ...
      <HStack w='md' display={{lg: 'flex', md: 'flex', sm: 'none', base: 'none'}}>
        <Heading size='md'>1ST = 50 USD = {STtoETH} ETH</Heading>
        <Spacer/>
      ...
      <HStack w='md' display={{lg: 'flex', md: 'flex', sm: 'none', base: 'none'}}>
        <Slider value={SliderValue()} h='50px'>
          <SliderTrack bg={sliderBG} h='25px' borderRadius={8}>
            ...
            <Center>
              <Text>{Number(tokensSold).toFixed()} / 750.000 ST</Text>
            </Center>
          </SliderTrack>
        </Slider>
      </HStack>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Buy ST</ModalHeader>
          <ModalCloseButton/>
          <ModalBody>
            <Input
            placeholder='1 ST'
...
const HeaderST = () => {
  return (
    <VStack>
       <Head>
        <title>S Token ICO</title>
        <meta name="description" content="STokenICO" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <Box h='5px'/>
      <HStack w='full'>
        <Spacer />
        <Heading size='2xl'>S Token ICO</Heading>
        <Spacer />
      </HStack>
      <Box h='10px'/>
    </VStack>
